...
async function main() {
  ...
  try {
    while (Date.now() < deadline) {
      ...
      const event = destination ? await prisma.paymentEvent.findUnique({ where: { providerEventId: destination.sourceEventId } }) : null;
      const destinationCreatedAt = destination?.createdAt.getTime() ?? Number.POSITIVE_INFINITY;
      lastState = JSON.stringify({ status: order.status, paid: Boolean(order.paidAt), destination: Boolean(destination), event: event?.type ?? null, intents: order.procurementIntents.length, items: order.items.length });

      const complete = order.status === "paid" && Boolean(order.paidAt) && Boolean(order.stripeCheckoutSessionId) && Boolean(order.stripePaymentIntentId) &&
        destination?.source === "stripe_checkout" && destination.stripeCheckoutSessionId === order.stripeCheckoutSessionId &&
        event?.type === "checkout.session.completed" && event.status === "processed" && event.orderId === order.id &&
        destination.name === expected.name && destination.line1 === expected.line1 && destination.city === expected.city &&
        destination.state === expected.state && destination.postalCode === expected.postalCode && destination.country === expected.country &&
        ...
        order.procurementIntents.every(intent => intent.executionMode === "manual_only" && intent.createdAt.getTime() >= destinationCreatedAt);

      if (complete && destination && event) {
        await writeFile("shipping-certification-evidence.json", JSON.stringify({
