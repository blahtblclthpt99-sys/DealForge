...
DealForge separates a stored catalog price from a current retailer price.
...
- **Recorded price**: a valid historical/catalog price. It may be displayed as a dollar amount only when clearly labeled as recorded/stale and accompanied by a retailer-price check message.
- **Unavailable price**: no valid positive stored price. The storefront sends the shopper to the retailer to check the price.
- Discounts, strike-through list prices, savings percentages, and price-alert decisions must use current trusted prices only.
...
Target, Best Buy, Home Depot, Lowe's, Wayfair, and other merchants may be added later only after their current affiliate relationship and deep-link mechanism are verified. Do not add a retailer merely because its public storefront exists.
...
7. Price presentation check: recorded prices must be visibly labeled; current prices require trusted freshness
8. Custom-domain smoke before declaring production complete
