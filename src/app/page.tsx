...
export default async function HomePage() {
  if (!isDatabaseConfigured()) {
    return <SetupBanner message="DealForge requires its production PostgreSQL connection before the live catalog can load." />;
  }
  ...
  const verifiedOffers = publicCatalogItems(flash.items).filter(
    (product) => product.priceVerified && product.metadataVerified && product.availability === "in_stock",
  );
    ...
    <div>
        ...
        <div className="dn-container relative py-12 md:py-18">
          <div className="max-w-3xl animate-fade-up">
            ...
            <h1 className="mt-3 font-display text-4xl font-semibold leading-[1.08] tracking-tight text-forest-ink md:text-6xl">
              Find what you want without guessing what&apos;s current.
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-forest-muted md:text-lg">
              DealForge separates current verified commerce signals from retailer-check listings, so price and stock claims are clear before you act.
            </p>
            ...
            <form action="/search" method="get" className="mt-7 flex max-w-2xl gap-2 rounded-2xl border border-card-border bg-card p-2 shadow-sm">
              ...
              <button type="submit" className="rounded-xl bg-forest px-5 py-3 text-sm font-semibold text-white transition hover:bg-forest-dark">
                Search
              </button>
            </form>

            <div className="mt-4 flex flex-wrap gap-2">
              <Link href="/categories" className="inline-flex items-center gap-2 rounded-full border border-card-border bg-card/80 px-4 py-2 text-sm font-semibold text-forest-ink hover:border-forest/40">
                Browse categories <ArrowRight className="h-3.5 w-3.5" />
              </Link>
              <Link href="/search" className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-forest hover:bg-forest/10">
                Shop all products
              </Link>
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-2 sm:grid-cols-3">
            <div className="flex items-center gap-2 rounded-xl border border-card-border bg-card/70 px-3 py-2.5 text-xs text-forest-muted backdrop-blur">
              <ShieldCheck className="h-4 w-4 shrink-0 text-forest" /> Verified claims are labeled clearly
            </div>
            <div className="flex items-center gap-2 rounded-xl border border-card-border bg-card/70 px-3 py-2.5 text-xs text-forest-muted backdrop-blur">
              <RefreshCw className="h-4 w-4 shrink-0 text-forest" /> Stale stock is not presented as current
            </div>
            <div className="flex items-center gap-2 rounded-xl border border-card-border bg-card/70 px-3 py-2.5 text-xs text-forest-muted backdrop-blur">
              <Sparkles className="h-4 w-4 shrink-0 text-forest" /> Retailer checks stay one tap away
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-card-border bg-card/40 py-10 md:py-12">
        <div className="dn-container">
          <SectionHeader title="Shop by category" subtitle="Jump straight to the aisle you need" href="/categories" />
      ...
      {popularItems.length > 0 && (
        <section className="dn-container py-10 md:py-12">
          <SectionHeader title="Popular now" subtitle="Products shoppers are exploring" href="/search?sort=popularity" />
          <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
            {popularItems.map((product) => <ProductCard key={product.id} product={product} />)}
          </div>
        </section>
      )}
      ...
      {newestItems.length > 0 && (
        <section className="dn-container py-10 md:py-12">
          <SectionHeader title="Newly added" subtitle="Recent additions to the DealForge catalog" href="/search?sort=newest" />
          <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
            {newestItems.map((product) => <ProductCard key={product.id} product={product} />)}
          </div>
        </section>
      )}

      {verifiedOffers.length > 0 && (
        <section className="border-y border-card-border bg-card/40 py-10 md:py-12">
          <div className="dn-container">
            <SectionHeader title="Verified offers" subtitle="Current price and availability signals passed DealForge checks" href="/deals" />
            <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
              {verifiedOffers.map((product) => <ProductCard key={product.id} product={product} />)}
            </div>
          </div>
        </section>
      )}
