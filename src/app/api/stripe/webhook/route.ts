...
import { payloadSha256, stripeWebhookSecret, verifyStripeSignature, type StripeEvent } from "@/lib/stripe-commerce";
...
function paymentIntentId(object: Record<string, unknown>) {
  ...
  if (raw && typeof raw === "object" && !Array.isArray(raw)) return asString((raw as Record<string, unknown>).id);
  return null;
}
...
async function resolveOrderId(tx: Prisma.TransactionClient, object: Record<string, unknown>) {
  ...
  const order = await tx.order.findUnique({ where: { stripePaymentIntentId: intentId }, select: { id: true } });
  return order?.id || null;
}

async function assertPaymentMatchesOrder(tx: Prisma.TransactionClient, orderId: string, object: Record<string, unknown>) {
...
async function markPaymentSucceeded(tx: Prisma.TransactionClient, orderId: string, object: Record<string, unknown>) {
  ...
  if (sessionId && order.stripeCheckoutSessionId && order.stripeCheckoutSessionId !== sessionId) throw new Error("WEBHOOK_CHECKOUT_SESSION_MISMATCH");
  if (order.stripePaymentIntentId && order.stripePaymentIntentId !== intentId) throw new Error("WEBHOOK_PAYMENT_INTENT_MISMATCH");
...
async function markPaymentFailed(tx: Prisma.TransactionClient, orderId: string, object: Record<string, unknown>) {
  ...
  if (intentId) {
    await tx.payment.upsert({
      where: { providerPaymentId: intentId },
      create: { orderId, providerPaymentId: intentId, providerSessionId: order.stripeCheckoutSessionId, status: "failed", amountCents: order.totalCents, currency: order.currency },
...
async function reconcileRefund(tx: Prisma.TransactionClient, object: Record<string, unknown>) {
  ...
  if (!refundId || !intentId || !amountCents || amountCents <= 0 || !currency) throw new Error("WEBHOOK_REFUND_INVALID");
  const metadata = metadataOf(object);
  const order = metadata.order_id ? await tx.order.findUnique({ where: { id: metadata.order_id } }) : await tx.order.findUnique({ where: { stripePaymentIntentId: intentId } });
...
export async function POST(request: Request) {
  ...
  let secret: string;
  try { secret = stripeWebhookSecret(); } catch { return NextResponse.json({ error: "WEBHOOK_NOT_CONFIGURED" }, { status: 503 }); }
  if (!verifyStripeSignature(rawBody, signature, secret)) return NextResponse.json({ error: "INVALID_STRIPE_SIGNATURE" }, { status: 400 });

  let event: StripeEvent;
  try {
    event = JSON.parse(rawBody) as StripeEvent;
    if (!event.id?.startsWith("evt_") || !event.type || !event.data?.object) throw new Error("INVALID_EVENT");
  } catch {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      await tx.paymentEvent.create({ data: { providerEventId: event.id, type: event.type, payloadSha256: payloadSha256(rawBody), status: "processed", orderId, processedAt: new Date() } });
    });
    ...
    console.error("stripe.webhook.failed", { eventId: event.id, type: event.type, error: error instanceof Error ? error.message : "UNKNOWN" });
