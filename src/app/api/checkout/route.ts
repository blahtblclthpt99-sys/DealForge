...
type OrderEconomicItem = {
  productId: string; quantity: number; unitPriceCents: number; lineTotalCents: number;
  landedCostCents: number | null; supplierSnapshot: string;
};
type CurrentEconomicItem = {
  product: { id: string; specifications: string; landedCostCents: number | null; currency: string; availability: string; priceVerifiedAt: Date | null };
  quantity: number; unitPriceCents: number; lineTotalCents: number; supplierSnapshot: string;
};
...
function sameOrderItems(existing: Array<{ productId: string; quantity: number }>, requested: Array<{ productId: string; quantity: number }>) {
  ...
  return left.every((item, index) => item.productId === requested[index].productId && item.quantity === requested[index].quantity);
}
function sameOrderEconomics(existing: OrderEconomicItem[], current: CurrentEconomicItem[]) {
  ...
  return existing.every((item) => {
    ...
    return Boolean(live && item.quantity === live.quantity && item.unitPriceCents === live.unitPriceCents && item.lineTotalCents === live.lineTotalCents && item.landedCostCents === live.product.landedCostCents && item.supplierSnapshot === live.supplierSnapshot);
  });
}
function commerceEnabled() { return process.env.COMMERCE_ENABLED === "true"; }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body.ok) {
      return NextResponse.json({ error: body.error === "BODY_TOO_LARGE" ? "CHECKOUT_BODY_TOO_LARGE" : "INVALID_CHECKOUT_REQUEST" }, { status: body.error === "BODY_TOO_LARGE" ? 413 : 400 });
    }
    ...
    const legacyCertificationAttempt = parsed.data.items.length > 0 && parsed.data.items.every((item) => item.productId === CERTIFICATION_PRODUCT_ID);
    const catalogCertificationAttempt = parsed.data.items.length > 0 && parsed.data.items.every((item) => isCertificationCatalogId(item.productId));
    ...
    if (existing) {
      if (existing.email !== email || !sameOrderItems(existing.items, requestedItems) || (existing.userId !== null && existing.userId !== sessionUser?.id)) {
        return NextResponse.json({ error: "CHECKOUT_KEY_CONFLICT" }, { status: 409 });
      }
      if (TERMINAL_CHECKOUT_STATUSES.has(existing.status)) {
        return NextResponse.json({ error: "ORDER_NOT_RETRYABLE", orderNumber: existing.orderNumber }, { status: 409 });
      }
    }
    ...
    const products = await prisma.product.findMany({
      ...
      select: { id: true, slug: true, title: true, specifications: true, commerceEnabled: true, sellingPriceCents: true, landedCostCents: true, priceVerifiedAt: true, currency: true, availability: true },
    });
    ...
    stage = "commerce_gate";
    const catalogCertificationOnly = isCertificationCatalogMode() && catalogCertificationAttempt && products.length > 0 && products.every(isCertificationCatalogProduct);
    const legacyCertificationOnly = legacyCertificationAttempt && products.length > 0 && products.every(isLegacyStripeCertificationProduct);
    const certificationOnly = catalogCertificationOnly || legacyCertificationOnly;
    if (isCertificationCatalogMode() && !catalogCertificationOnly) return NextResponse.json({ error: "PRODUCT_NOT_IN_CERTIFICATION_CATALOG" }, { status: 409 });
    ...
    for (const product of products) {
      ...
      const input = { commerceEnabled: product.commerceEnabled, availability: product.availability, sellingPriceCents: product.sellingPriceCents, landedCostCents: product.landedCostCents, priceVerifiedAt: product.priceVerifiedAt, specifications: product.specifications };
      const decision = catalogCertificationOnly ? evaluateCertificationCommerceGate(input) : evaluateCommerceGate(input);
      if (!decision.allowed) {
        console.warn("checkout.commercial_gate.blocked", { productId: product.id, reasons: decision.reasons, contributionProfitCents: decision.contributionProfitCents, contributionMarginBps: decision.contributionMarginBps });
    ...
    const pricedItems = requestedItems.map((item) => {
      ...
      if (internalCertification && !legacyCertificationProduct && !catalogCertificationProduct) throw new Error("CERTIFICATION_PRODUCT_NOT_AUTHORIZED");
      if (internalCertification && !isStripeTestMode()) throw new Error("CERTIFICATION_REQUIRES_TEST_MODE");
      ...
      if (!legacyCertificationProduct) {
        if (!Number.isSafeInteger(product.landedCostCents) || !product.landedCostCents || product.landedCostCents <= 0) throw new Error(`PRODUCT_NOT_PURCHASABLE:${product.id}`);
        ...
        if (!pricing.eligible) throw new Error("PRODUCT_PRICE_NO_LONGER_SAFE");
      ...
      if (supplierSnapshot === undefined) throw new Error("SUPPLIER_SNAPSHOT_MISSING");
      return { product, quantity: item.quantity, unitPriceCents, lineTotalCents, supplierSnapshot };
    });
    ...
    stage = "exposure_gate";
    const exposure = checkCheckoutExposure(pricedItems.map(({ product, quantity, unitPriceCents }) => ({ quantity, unitPriceCents, landedCostCents: product.landedCostCents })));
    if (!exposure.eligible) return NextResponse.json({ error: "CHECKOUT_LIMIT_EXCEEDED" }, { status: 409 });

    stage = "order_create";
    const order = existing || (await prisma.order.create({
      data: {
        orderNumber: orderNumber(), checkoutKey: parsed.data.checkoutKey, userId: sessionUser?.id || null, email, currency,
        status: "pending_payment", subtotalCents, totalCents: subtotalCents,
        items: { create: pricedItems.map(({ product, quantity, unitPriceCents, lineTotalCents, supplierSnapshot }) => ({ productId: product.id, productSlug: product.slug, title: product.title, quantity, unitPriceCents, lineTotalCents, landedCostCents: product.landedCostCents, supplierSnapshot })) },
      },
      include: { items: true },
    }));

    stage = "order_integrity";
    if (order.currency !== currency || order.subtotalCents !== subtotalCents || order.totalCents !== subtotalCents || !sameOrderItems(order.items, requestedItems) || !sameOrderEconomics(order.items, pricedItems)) {
    ...
    if (message.startsWith("PRODUCT_NOT_PURCHASABLE") || message === "PRODUCT_PRICE_NO_LONGER_SAFE" || message === "QUANTITY_LIMIT_EXCEEDED" || message === "CERTIFICATION_REQUIRES_TEST_MODE" || message === "CERTIFICATION_PRODUCT_NOT_AUTHORIZED") {
      return NextResponse.json({ error: message.split(":")[0] }, { status: 409 });
    }
    console.error("checkout.create.failed", { error: message, stage });
    return NextResponse.json(certificationAttempt ? { error: "CHECKOUT_UNAVAILABLE", stage } : { error: "CHECKOUT_UNAVAILABLE" }, { status: 503 });
  }
}