...
const actionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("publish"), candidateId: z.string().min(1).max(100) }),
  z.object({ action: z.literal("retry"), candidateId: z.string().min(1).max(100) }),
]);

async function requireOwner() {
  const session = await requireAdmin();
  const user = await prisma.user.findUnique({ where: { id: session.id }, select: { id: true, email: true, role: true } });
...
export async function POST(req: Request) {
  try {
    ...
    const status = ["INVALID_ASIN", "INVALID_SOURCE_TYPE", "INVALID_AMAZON_SPECIAL_LINK", "SPECIAL_LINK_ASIN_MISMATCH", "SOURCE_URL_SCHEME", "SOURCE_URL_PRIVATE_HOST", "NOT_RETRYABLE"].includes(message) ? 400 : 500;
