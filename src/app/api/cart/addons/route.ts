...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "INVALID_ADDON_SEARCH" }, { status: 400 });

    const cartIds = [...new Set(parsed.data.items.map((item) => item.productId))];
    const cartProducts = await prisma.product.findMany({
      where: { id: { in: cartIds } },
      select: {
        id: true, brand: true, categoryId: true, subcategory: true, currency: true,
        sellingPriceCents: true, landedCostCents: true, availability: true,
        commerceEnabled: true, priceVerifiedAt: true, specifications: true,
      },
    });
    if (cartProducts.length !== cartIds.length) return NextResponse.json({ error: "PRODUCT_NOT_FOUND" }, { status: 409 });
    ...
    if (currentSafePrices.length === 0) return NextResponse.json({ items: [], reason: "CART_NOT_ELIGIBLE_FOR_ADDONS" });

    const averageCartUnitCents = Math.round(currentSafePrices.reduce((sum, value) => sum + value, 0) / currentSafePrices.length);
    ...
    const candidates = await prisma.product.findMany({
      where: {
        id: certificationMode ? { in: [...CERTIFICATION_CATALOG_PRODUCT_IDS], notIn: cartIds } : { notIn: cartIds },
        categoryId: { in: categoryIds }, currency, commerceEnabled: true, availability: "in_stock",
        sellingPriceCents: { not: null }, landedCostCents: { not: null },
      },
      select: {
        id: true, slug: true, title: true, brand: true, categoryId: true, subcategory: true, currency: true,
        commerceEnabled: true, availability: true, sellingPriceCents: true, landedCostCents: true,
        priceVerifiedAt: true, specifications: true,
      },
    ...
    const eligible: Array<{
      productId: string; slug: string; title: string; brand: string; currency: string;
      unitPriceCents: number; publishedUnitPriceCents: number; savingsCents: number;
      fitLabel: string; affinityScore: number;
    }> = [];
    ...
    for (const product of candidates) {
      ...
      const gateInput = {
        commerceEnabled: product.commerceEnabled, availability: product.availability,
        sellingPriceCents: product.sellingPriceCents, landedCostCents: product.landedCostCents,
        priceVerifiedAt: product.priceVerifiedAt, specifications: product.specifications,
      };
      const gate = certificationTransaction ? evaluateCertificationCommerceGate(gateInput) : evaluateCommerceGate(gateInput);
      if (!gate.allowed) continue;

      if (!certificationTransaction) {
        const binding = await checkPersistedOfferBinding({
          productId: product.id, currency: product.currency, availability: product.availability,
          landedCostCents: product.landedCostCents, priceVerifiedAt: product.priceVerifiedAt,
      ...
      eligible.push({
        productId: product.id, slug: product.slug, title: product.title, brand: product.brand,
        currency: product.currency.toLowerCase(), unitPriceCents: pricing.customerPriceCents,
        publishedUnitPriceCents: product.sellingPriceCents, savingsCents: pricing.savingsCents,
        fitLabel, affinityScore,
      });
    }

    eligible.sort((a, b) => b.affinityScore - a.affinityScore || a.unitPriceCents - b.unitPriceCents);
    return NextResponse.json({
      items: eligible.slice(0, MAX_RESULTS), maxAddonPriceCents: addonPriceCapCents, currency,
