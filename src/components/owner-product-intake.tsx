      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm leading-6 text-forest-muted">
            Paste an Amazon ASIN or full Amazon product URL and tap Add ASIN. If approved retailer data is available, DealForge enriches it immediately. Otherwise it stays in your private queue and never appears on the storefront as an unfinished listing.
          </p>
        </div>
        <button
          type="button"
      ...
      >
        <label className="min-w-0 flex-1">
          ...
          <input
            ...
            className="min-h-11 w-full rounded-xl border border-card-border bg-background px-4 py-2.5 text-sm text-forest-ink outline-none focus:border-[#F97316]/60 focus:ring-2 focus:ring-[#F97316]/15"
          />
        </label>
        <button
          type="submit"
          disabled={!value.trim() || busy !== null}
          className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#F97316] px-5 py-2.5 text-sm font-bold text-white shadow-[0_8px_22px_rgba(249,115,22,.18)] transition hover:bg-[#EA580C] disabled:cursor-not-allowed disabled:opacity-50"
        >
          <Plus className="h-4 w-4" />
          {busy === "add" ? "Adding…" : "Add ASIN"}
        </button>
      </form>
