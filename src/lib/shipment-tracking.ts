...
export type DeliveryRecordedV1 = { version: 1; deliveredAt: string };
export type DeliveryRecordedV2 = { version: 2; packageId: string; deliveredAt: string };
export type DeliveryRecorded = DeliveryRecordedV1 | DeliveryRecordedV2;

type ProcurementJournalEvent = { type: string; detail: string; createdAt?: Date | string };
...
export type ShipmentJournalSummary =
  | { ok: true; packages: PublicShipmentPackage[]; shippedQuantity: number; deliveredQuantity: number }
  | { ok: false; reason: "SHIPMENT_JOURNAL_INVALID" | "SHIPMENT_PACKAGE_DUPLICATE" | "DELIVERY_PACKAGE_UNKNOWN" | "DELIVERY_PACKAGE_DUPLICATE" | "DELIVERY_TIMESTAMP_INVALID" };

const CARRIER_NAMES: Record<Exclude<TrackingCarrier, "other">, string> = { ups: "UPS", usps: "USPS", fedex: "FedEx", dhl: "DHL" };
...
export function resolveTrackingCarrier(carrierCode: unknown, carrierName: unknown): { carrierCode: TrackingCarrier; carrierName: string } | null {
  if (typeof carrierCode !== "string" || !(TRACKING_CARRIERS as readonly string[]).includes(carrierCode)) return null;
...
export function buildOfficialTrackingUrl(carrierCode: TrackingCarrier, trackingNumber: string): string | null {
  ...
  switch (carrierCode) {
    ...
    case "dhl": return `https://www.dhl.com/global-en/home/tracking/tracking-express.html?submit=1&tracking-id=${encoded}`;
    case "other": return null;
  }
}

export function shipmentPackageId(carrierCode: TrackingCarrier, trackingNumber: string) {
  const digest = createHash("sha256").update(`${carrierCode}:${trackingNumber}`).digest("hex").slice(0, 24);
  return `pkg_${digest}`;
}

function normalizedShipmentFields(input: { carrierCode: unknown; carrierName?: unknown; trackingNumber: unknown; quantity: unknown; shippedAt?: unknown }) {
  const carrier = resolveTrackingCarrier(input.carrierCode, input.carrierName);
  const trackingNumber = normalizeTrackingNumber(input.trackingNumber);
  if (!carrier || !trackingNumber || !Number.isSafeInteger(input.quantity) || (input.quantity as number) <= 0) return null;
  const shippedAt = input.shippedAt === undefined ? new Date().toISOString() : safeTimestamp(input.shippedAt);
  if (!shippedAt) return null;
  return { carrierCode: carrier.carrierCode, carrierName: carrier.carrierName, trackingNumber, trackingUrl: buildOfficialTrackingUrl(carrier.carrierCode, trackingNumber), quantity: input.quantity as number, shippedAt };
}

export function createShipmentRecord(input: { carrierCode: unknown; carrierName?: unknown; trackingNumber: unknown; quantity: unknown; shippedAt?: unknown }): ShipmentRecordedV2 | null {
  const normalized = normalizedShipmentFields(input);
  if (!normalized) return null;
  return { version: 2, packageId: shipmentPackageId(normalized.carrierCode, normalized.trackingNumber), ...normalized };
}
...
export function parseShipmentEventDetail(raw: string): ShipmentRecorded | null {
  try {
    ...
    const normalized = normalizedShipmentFields({ carrierCode: value.carrierCode, carrierName: value.carrierName, trackingNumber: value.trackingNumber, quantity: value.quantity, shippedAt: value.shippedAt });
    if (!normalized || value.trackingUrl !== normalized.trackingUrl) return null;
    if (value.version === 1) return { version: 1, ...normalized };
...
export function createDeliveryRecord(input: { packageId: unknown; deliveredAt?: unknown }): DeliveryRecordedV2 | null {
...
export function parseDeliveryEventDetail(raw: string): DeliveryRecorded | null {
  try {
    ...
    if (value.version !== 2 || typeof value.packageId !== "string" || !/^pkg_[a-f0-9]{24}$/.test(value.packageId)) return null;
    return { version: 2, packageId: value.packageId, deliveredAt };
  } catch { return null; }
}

export function packageIdForShipment(shipment: ShipmentRecorded) {
  return shipment.version === 2 ? shipment.packageId : shipmentPackageId(shipment.carrierCode, shipment.trackingNumber);
}
...
export function summarizeShipmentJournal(events: ProcurementJournalEvent[]): ShipmentJournalSummary {
  ...
  for (const event of events) {
    if (event.type === "RECORD_SHIPMENT") {
      ...
      packages.set(packageId, { packageId, status: "shipped", carrierName: shipment.carrierName, trackingNumber: shipment.trackingNumber, trackingUrl: shipment.trackingUrl, quantity: shipment.quantity, shippedAt: shipment.shippedAt, deliveredAt: null });
    ...
    if (Date.parse(delivery.deliveredAt) < Date.parse(shipment.shippedAt)) return { ok: false, reason: "DELIVERY_TIMESTAMP_INVALID" };
...
export function projectPublicShipments(events: ProcurementJournalEvent[], expectedQuantity?: number) {
  if (expectedQuantity !== undefined && (!Number.isSafeInteger(expectedQuantity) || expectedQuantity <= 0)) return [];
...
export function projectPublicShipment(events: ProcurementJournalEvent[], expectedQuantity?: number) {
  ...
  if (expectedQuantity !== undefined && (!Number.isSafeInteger(expectedQuantity) || expectedQuantity <= 0 || shipment.quantity !== expectedQuantity)) return null;
  ...
  return { status: delivery ? ("delivered" as const) : ("shipped" as const), carrierName: shipment.carrierName, trackingNumber: shipment.trackingNumber, trackingUrl: shipment.trackingUrl, shippedAt: shipment.shippedAt, deliveredAt: delivery?.deliveredAt ?? null };
}
