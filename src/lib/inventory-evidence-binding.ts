...
function normalizeEvidence(value: unknown): InventoryEvidenceV1 | null {
  ...
  const inventoryConfidenceBps = typeof raw.inventoryConfidenceBps === "number" && Number.isSafeInteger(raw.inventoryConfidenceBps) && raw.inventoryConfidenceBps >= 0 && raw.inventoryConfidenceBps <= 10_000
    ? raw.inventoryConfidenceBps
    : null;
...
function sameEvidence(left: InventoryEvidenceV1, right: InventoryEvidenceV1) {
  return (
    ...
    left.supplierOfferId === right.supplierOfferId &&
    left.idempotencyKey === right.idempotencyKey &&
    left.availability === right.availability &&
    left.quantity === right.quantity &&
    left.inventoryConfidenceBps === right.inventoryConfidenceBps &&
    left.observedPriceCents === right.observedPriceCents &&
    left.observedAt === right.observedAt &&
    left.expiresAt === right.expiresAt &&
    left.verificationMethod === right.verificationMethod &&
    left.provenanceHash === right.provenanceHash &&
    left.sourceHealth === right.sourceHealth
  );
}
