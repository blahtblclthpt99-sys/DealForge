...
type ShipmentJournalEvent = { eventKey: string; detail: string };
...
function uniqueReasons(reasons: ShipmentJournalIntegrityReason[]) { return [...new Set(reasons)]; }
...
export function reconcileShipmentJournal(input: {
  ...
  if (input.events.length === 0) {
    return { ok: false, reasons: ["shipment_event_missing"], shipment: null, shipments: [], shippedQuantity: 0, purchaseEvidenceHash: null };
  }
  ...
  for (const event of input.events) {
    ...
    if (!shipment) {
      ...
      if (packageIds.has(packageId)) reasons.push("shipment_package_duplicate");
      else {
    ...
    if (!purchaseEvidenceHash) reasons.push("shipment_purchase_evidence_missing");
    else {
      purchaseEvidenceHashes.push(purchaseEvidenceHash);
      if (purchaseEvidenceHash !== input.expectedPurchaseEvidenceHash.toLowerCase()) reasons.push("shipment_purchase_evidence_mismatch");
    }
  }

  if (!Number.isSafeInteger(input.expectedQuantity) || input.expectedQuantity <= 0) reasons.push("shipment_quantity_mismatch");
  else if (shippedQuantity > input.expectedQuantity) reasons.push("shipment_quantity_exceeded");
  else if (!input.allowPartial && shippedQuantity !== input.expectedQuantity) reasons.push("shipment_quantity_mismatch");

  const normalized = uniqueReasons(reasons);
  const commonPurchaseEvidenceHash = purchaseEvidenceHashes.length === input.events.length && purchaseEvidenceHashes.every((value) => value === purchaseEvidenceHashes[0]) ? purchaseEvidenceHashes[0] : null;
  return { ok: normalized.length === 0, reasons: normalized, shipment: shipments.length === 1 ? shipments[0] : null, shipments, shippedQuantity, purchaseEvidenceHash: commonPurchaseEvidenceHash };
}
